...
const Home = () => {
  ...
  useEffect(() => {
    axios
      .get(
        `https://jobtask-servire.vercel.app/productes?page=${carentpage}&size=${totalparpage}&categorys=${categorys}&serch=${serch}&priceShort=${priceShort}`
      )
      .then((data) => {
  ...
  return (
    <section className="w-[90%] mx-auto px-5 py-36">
      <div className="flex flex-wrap justify-center mb-10 md:mb-16 gap-10">
        ...
        <div className="">
          <form
            ...
            className="flex relative rounded-md w-full px-4 max-w-xl"
          >
            <input
              type="text"
              name="secarch"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {allproducts.length > 0 &&
          allproducts.map((product, index) => (
            ...
            <div
              key={index}
              className=" duration-500 group overflow-hidden relative rounded bg-neutral-800 text-neutral-50 p-4 flex flex-col justify-evenly"
            >
                ...
                <div className="z-10 ">
                  <div className="">
                    <img
                      ...
                      className="product-image w-48 object-cover mx-auto"
                    />
                  </div>
                  <div className="p-5">
                    <h2 className="product-name text-xl mt-5 font-semibold">
                    ...
                    <p className="product-ratings">
                      <span className="text-xl font-medium">Ratings: </span>{" "}
                      {product.ratings}
                    </p>
                  </div>
                  ...
                  >
                    <span className="sp">Details</span>
                  </button>
                </div>
              </div>
            </div>
        ...
        <dialog id="my_modal_1" className="modal">
          <div className="modal-box">
            <div className="flex justify-between items-center">
              <h3 className="font-bold text-lg mb-2 mt-4">Product Details</h3>
              <div className="modal-action">
                <form method="dialog">
                  <button className="bg-transparent border-none shadow-none -mt-4">
                    <RxCross2 className="text-3xl rounded-sm border-black border  font-bold" />
                  </button>
                </form>
              </div>
            </div>
            <img
              ...
              className="product-image md:w-64 object-cover mx-auto"
            />
            <p className="py-4">
      ...
      {allproducts.length === 0 && (
        <div className="">
          <h1 className="uppercase text-center text-2xl font-bold pb-5">
            data not found
          </h1>
          <img
            className="bg-cover rounded-md bg-center w-full h-full"
            src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRoL8RBNTbZGJ49Ah8qlbqaKs7PVkbnJ1Ij9w&usqp=CAU"
            alt="Not Found"
          />
        </div>
      )}
      ...
      <div className="flex justify-center  md:mt-10 items-center mt-10">
        <button onClick={handelprevious} className="ctas items-center">
        ...
        {pages.map((page) => (
          <button
            className={carentpage === page ? "buttons" : "Btn"}
            key={page}
            onClick={() => setCarrentPage(page)}
          >
            {page + 1}
          </button>
        ))}
        <select
          onChange={handelDainamicpage}
          className="block w-sm border-2 text-sm font-medium px-1 ml-2  transition duration-75  border-gray-800 rounded-lg shadow-sm h-9 focus:border-blue-600 focus:ring-1 focus:ring-inset focus:ring-blue-600 bg-none"
        >
        ...
        <button onClick={handelNext} className="cta items-center">
